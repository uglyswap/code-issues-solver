      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-900 text-white">
        ...
        <nav className="flex-1 px-2 space-y-1">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} className={`flex items-center gap-3 px-3 py-2 rounded-lg transition ${active ? 'bg-primary-600' : 'hover:bg-slate-800'}`}>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-slate-900 text-white pt-16 px-4">
          <nav className="space-y-2">
            {navItems.map((item) => (
              <Link key={item.path} to={item.path} onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-800">
                <item.icon className="w-5 h-5" />
                {item.label}
              </Link>
            ))}
