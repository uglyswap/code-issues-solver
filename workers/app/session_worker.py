...
from backend.app.database import async_session as get_db
from backend.app import models, crud
from backend.app.security import decrypt_value
from workers.app.tasks import generate_patch, review_patch, verify_fix, _log
from integrations.app.github_client import GitHubClient

# Setup broker
broker = RedisBroker(url="redis://redis:***@dramatiq.actor(max_retries=0)
...
async def _run_continuous_session_async(session_id: int):
    ...
    async with get_db() as db:
        ...
        while True:
            ...
            if not open_tickets:
                # Check if there are any tickets in progress
                in_progress = await _get_in_progress_tickets(db, project.id)
                if not in_progress:
                    # No tickets at all - session complete!
            ...
            # Process tickets one by one (sequential for now, can be parallelized later)
            for ticket in open_tickets:
                ...
                # Process the ticket through the full pipeline
...
async def _get_open_tickets(db, project_id: int) -> list[models.Ticket]:
    ...
    result = await db.execute(
        ...
        .order_by(
            # Priority: high severity first, then oldest first
            case(
...
async def _process_ticket(db, session_id: int, ticket: models.Ticket, project: models.Project) -> bool:
    ...
    try:
        ...
        # Call generate_patch synchronously (it's a dramatiq actor but we need to wait)
        ...
        # Step 4: Deploy (if webhook configured)
